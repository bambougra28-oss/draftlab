...
| 15 | **Win-condition scaling axis: replace the tag signal with time data.** r = 0.006 between tag-late counts and real 2026 game length (and the "later" side wins SHORT games 56.7 %) — `scalingWindow` does not track pro tempo, which is the root cause of the G1 chance result. Axis 5 should consume statsByTime / corpus duration profiles as primary, tag as fallback. | draft science 2026-06 §G | medium (axis rework) | **applied (2026-06-10)** — power curve = signal primaire (tags amortis ×0.25 sous courbe, config `tagWeightWithCurve`), repli tags-seuls plafonné à confiance `low`. **Re-mesure G1 le soir même** (piste courbes PRO walk-forward, `$lib/estimators/proPowerCurves`, priors 12/50 pré-enregistrés) : encore au hasard (48,1 % [43,8 ; 52,5], n=511) — le mécanisme est en place, le signal manque. **Piste compo-niveau** (`$lib/estimators/compDurationAffinity`, cellules paires de traits × durée, pré-enregistrée le soir même) : **52,1 % [49,1 ; 55,1] n=1059** — première piste qui penche (4 ligues ≥ 51 %), non significative, et la tranche haute-confiance (49,9 %) ne suit pas la magnitude. Suite : réplication règle gelée sur 2025 |
